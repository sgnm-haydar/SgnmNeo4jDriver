...
export class Neo4jService implements OnApplicationShutdown, OnModuleInit {
  ...
  async addRelations(
    ...
    try {
      ...
      cyper = `MATCH (n) where id(n)= $first_node_id MATCH (m) where id(m)= $second_node_id MERGE (n)<-[r:PARENT_OF {isDeleted:false, isCanceled:false}]- (m) return n as parent,m as children,r as relation`;
      const parameters = {
        first_node_id,
        second_node_id,
      };
  ...
  async findChildrensByIdAndFiltersWithChildrenOfChildrens3Criteria_V2(
    main_root_id: number,
    main_root_labels: string[] = [""],
    main_root_filters: object = {},
    main_relation_name: string,
    main_relation_filters: object = {},
    main_relation_depth: number | "",
    mainReverseRelation: boolean = false,
    root_labels: string[] = [""],
    root_filters: object = {},
    root_exculuded_labels: string[] = [""],
    children_labels: Array<string> = [],
