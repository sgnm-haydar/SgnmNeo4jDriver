...
export class Neo4jService implements OnApplicationShutdown {
  ...
  findNodeWithChilderen(id:string, databaseOrTransaction?: string | Transaction): Result {
    ...
    if (databaseOrTransaction instanceof TransactionImpl) {
      return (<Transaction>databaseOrTransaction).run(cypher, +id);
    }

    const session = this.getWriteSession(<string>databaseOrTransaction);
    const result = session.run(cypher, +id);
